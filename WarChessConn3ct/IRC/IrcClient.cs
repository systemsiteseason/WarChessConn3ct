...
namespace WarChessConn3ct.IRC
{
    /// <summary>
    /// IRC Client class edit by memmo
    /// </summary>
    public class IrcClient : IDisposable
    {
        ...
        public IrcClient(string Server, string ServerPass) : this(Server, 6667, false, ServerPass) { }
        ...
        public IrcClient(string Server, int Port, bool EnableSSL, string ServerPass)
        {
                ...
                case "366": // end list
                    {
                        ...
                        cnl = channel;
                        Fire_UpdateUsers(new UpdateUsersEventArgs(channel, playerList.ToArray()));
                        cnt = playerList.ToArray().Length;
                        playerList.Clear();
                    }
                    break;
                case "433":
                ...
                case "PRIVMSG": // message was sent to the channel or as private
                    {
                        ...
                        var to = ircData[2];
                        var checkcommand = ircData[3];
                        var message = JoinArray(ircData, 3);
                        /* var loop = true;
                         string[] list = new string[] { ":!pub", ":@pub", ":>pub", ":$pub", ":%pub", ":^pub", ":&pub", ":*pub", 
                         ":!load", ":@load", ":>load", ":$load", ":%load", ":^load", ":&load", ":*load",
                         ":!getgames", ":@getgames", ":>getgames", ":$getgames", ":%getgames", ":^getgames", ":&getgames", ":*getgames", 
                         ":!unhost", ":@unhost", ":>unhost", ":$unhost", ":%unhost", ":^unhost", ":&unhost", ":*unhost" };*/
                        /*for (int i = 0; i < list.Length; i++)
                        {
                            if (checkcommand != list[i])
                                loop = false;
                            else
                                loop = true;
                        }*/
                        // if it's a private message
                        if (String.Equals(to, _nick, StringComparison.CurrentCultureIgnoreCase)/* && loop == false*/)
                            Fire_PrivateMessage(new PrivateMessageEventArgs(from, message));
                        else if (checkcommand != ":!pub" && checkcommand != ":@pub" && checkcommand != ":>pub" && checkcommand != ":$pub" && checkcommand != ":%pub"
                        && checkcommand != ":^pub" && checkcommand != ":&pub" && checkcommand != ":*pub" && checkcommand != ":!load" && checkcommand != ":@load" && checkcommand != ":>load"
                        && checkcommand != ":$load" && checkcommand != ":%load" && checkcommand != ":^load" && checkcommand != ":&load" && checkcommand != ":*load" && checkcommand != ":!getgames"
                        && checkcommand != ":@getgames" && checkcommand != ":>getgames" && checkcommand != ":$getgames" && checkcommand != ":%getgames" && checkcommand != ":^getgames"
                        && checkcommand != ":&getgames" && checkcommand != ":*getgames" && checkcommand != ":!unhost" && checkcommand != ":@unhost" && checkcommand != ":>unhost"
                        && checkcommand != ":$unhost" && checkcommand != ":%unhost" && checkcommand != ":^unhost" && checkcommand != ":&unhost" && checkcommand != ":*unhost")
                        {
